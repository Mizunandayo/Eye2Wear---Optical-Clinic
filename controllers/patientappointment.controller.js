    ...
    const storage = multer.diskStorage({
        destination: (req, file, cb) => {
            const uploadPath = path.join(process.cwd(), 'uploads', 'appointment-documents');
    ...
    // File filter to allow only specific file types (no videos)
    const fileFilter = (req, file, cb) => {
        ...
        if (allowedTypes.includes(file.mimetype)) {
            ...
            cb(new Error(`File type ${file.mimetype} is not allowed. Please upload images or documents only (no videos).`), false);
        }
    };
            ...
            if (req.files && req.files.length > 0) {
                req.files.forEach((file) => {
                    supportingDocuments.push({
                        ...
                        size: file.size,
                        url: `/uploads/appointment-documents/${file.filename}`,
                        uploaddate: new Date()
                    });
                });
            }

            // Add supporting documents to the appointment data
            const appointmentData = {
                ...req.body,
                patientsupportingdocuments: supportingDocuments
            };

            const newpatientappointment = new PatientAppointment(appointmentData);
            const savedpatientappointment = await newpatientappointment.save();
            
            res.status(201).json({
                ...savedpatientappointment.toObject(),
                message: `Appointment created successfully with ${supportingDocuments.length} supporting documents`
            });
