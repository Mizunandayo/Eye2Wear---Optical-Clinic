...
mapStyles.textContent = `
  ...
  /* Also target the actual mapbox container when fullscreen */
  ...
  /* Prevent body scrolling during fullscreen */
...
function PatientLandingpage(){
 ...
 const {handlelogout, fetchpatientdetails} = useAuth();
  ...
  // Map functionality for patient view
  const apiUrl = import.meta.env.VITE_API_URL;
  
  // Memoized token and user info for patients
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          setLocationMessage({ 
            text: `Loaded ${activeClinics.length} clinic locations`, 
            type: 'success' 
          });
        } else {
  ...
  useEffect(() => {
    // Make getUserLocation function globally accessible for popup buttons
          ...
          <div className=" h-[90%] w-full flex items-center px-18">
            ...
            <div id="mappingintegration" className="flex flex-col pl-5 pr-5 pb-3 pt-4 transition-all duration-300 ease-in-out border-1 bg-white border-gray-200 shadow-lg w-[100%] min-h-full h-auto rounded-2xl">
              ...
              <div className="transition-all duration-300 ease-in-out gap-4 flex justify-center items-start flex-1 h-[580px]">
                ...
                {/* Clinic Locations List Container */}
