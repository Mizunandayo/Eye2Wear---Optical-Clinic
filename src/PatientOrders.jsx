 ...
 */}
    ...
    <div className="bg-cover bg-center h-auto w-full flex  justify-center " >
      ...
      <div className="w-full h-auto flex flex-col justify-start items-start pt-3 p-3">
              ...
              <div id="patientordersmanagement" className="px-2 sm:px-4 lg:px-5 pb-4 pt-8 transition-all duration-300 ease-in-out w-full h-full bg-white">   

              <div className="flex items-center mt-8"><i className="bx bxs-package text-[#184d85] text-[20px] sm:text-[25px] mr-2"/> <h1 className="font-albertsans font-bold text-[#184d85] text-[20px] sm:text-[25px]">My Orders</h1></div>

  <div className="flex flex-col sm:flex-row justify-start items-start sm:items-center mt-3 gap-2 sm:gap-0 sm:h-[60px]">
  <div onClick={() => showorderstable('ambherorderstable')} className={`w-full sm:w-auto sm:mr-3 mb-2 sm:mb-0 hover:rounded-2xl transition-all duration-300 ease-in-out border-2 b-[#909090] rounded-3xl px-4 sm:px-6 md:px-8 lg:px-12 xl:px-25 py-3 text-center flex justify-center items-center ${activeorderstable ==='ambherorderstable' ? 'bg-[#2781af] rounded-2xl' : ''}`}><h1 className={`font-albertsans font-semibold text-sm sm:text-base text-[#1f1f1f] ${activeorderstable ==='ambherorderstable' ? 'text-white' : ''}`}>Ambher Optical <span className="bg-gray-200 text-gray-500 font-semibold px-2 rounded-full ml-2 text-xs sm:text-sm"> {ambherOrders.length} </span></h1></div>
  <div onClick={() => showorderstable('bautistaorderstable')} className={`w-full sm:w-auto sm:ml-3 hover:rounded-2xl transition-all duration-300 ease-in-out border-2 b-[#909090] rounded-3xl px-4 sm:px-6 md:px-8 lg:px-12 xl:px-25 py-3 text-center flex justify-center items-center ${activeorderstable ==='bautistaorderstable' ? 'bg-[#2781af] rounded-2xl' : ''}`}><h1 className={`font-albertsans font-semibold text-sm sm:text-base text-[#1f1f1f] ${activeorderstable ==='bautistaorderstable' ? 'text-white' : ''}`}>Bautista Eye Center <span className="bg-gray-200 text-gray-500 font-semibold px-2 rounded-full ml-2 text-xs sm:text-sm"> {bautistaOrders.length} </span></h1></div>
          ...
          { activeorderstable === 'ambherorderstable' && ( <div id="ambherorderstable" className="p-2 animate-fadeInUp flex flex-col lg:flex-row items-start w-full min-h-[80] h-auto rounded-2xl mt-5" >
                ...
                <div className="pb-3 flex items center w-full mt-7"><i className="bx bx-filter font-albertsans font-semibold text-[#363636] text-[20px] sm:text-[25px]" /><h1 className="ml-2 text-[14px] sm:text-[16px] font-albertsans font-semibold text-[#363636]">Filter by status</h1></div>
                ...
                {['All', 'Pending', 'Ready for Pickup', 'Completed'].map(status => {
                    ...
                    return (
                       <div key={status} onClick={() => {setfilterambherorderedproductsStatus(status); setsearchpatientorderedProducts('');}} className={`mt-3 hover:rounded-2xl transition-all duration-300 ease-in-out border-2 b-[#909090] rounded-3xl py-2 text-center flex justify-center items-center cursor-pointer ${filterambherorderedproductsStatus === status ? 'bg-[#2781af] rounded-2xl text-white' : ''}`} >
                        <h1 className={`font-albertsans font-semibold text-sm sm:text-base ${filterambherorderedproductsStatus === status ? 'text-white' : 'text-[#1f1f1f]'}`}>{status} <span className="bg-gray-200 text-gray-500 font-semibold px-2 rounded-full ml-2 text-xs"> {patientorderedstatusCount} </span> </h1>  </div>
                        );})}
          ...
          {/* Orders Grid - Full width on mobile, 80% on desktop */}
          <div className="flex flex-col justify-start items-start ml-0 lg:ml-2 rounded-2xl w-full lg:w-[80%] min-h-[540px] max-h-auto h-auto shadow-b-lg">
              <div className="ml-2 flex flex-col lg:flex-row justify-start lg:justify-center items-start lg:items-center w-full"><h2 className="font-albertsans font-bold text-[16px] lg:text-[18px] text-[#383838] mr-0 lg:mr-3 mb-2 lg:mb-0">Search: </h2><div className="relative flex items-center justify-center gap-3 w-full lg:w-auto"><i className="bx bx-search absolute left-3 text-xl lg:text-2xl text-gray-500"></i><input value={searchpatientorderedProducts} onChange={(e) => handleSearch(e.target.value)} type="text" placeholder="Enter ordered product name here..." className="transition-all duration-300 ease-in-out py-2 pl-10 w-full lg:w-250 rounded-2xl bg-[#e4e4e4] focus:bg-slate-100 focus:outline-sky-500"></input></div></div>
              <div className="mt-5 w-full rounded-2xl h-auto flex flex-col gap-3 p-2 lg:pl-2 lg:pt-2 lg:pr-30">
                  ...
                  <div key={order.patientorderambherid} onClick={() => handleViewOrder(order)} className="pb-7 shadow-md rounded-2xl py-3.25 px-3.25 mb-3 border-1 flex flex-col sm:flex-row items-start sm:items-center motion-preset-slide-up w-full h-auto cursor-pointer hover:shadow-lg transition-all duration-300">
                   <img src={order.patientorderambherproductimage?.[0] || 'default-image-url'} alt={order.patientorderambherproductname} className="mr-0 sm:mr-5 w-full sm:w-35 h-32 sm:h-35 rounded-2xl mb-4 sm:mb-0 object-cover"/>
                    <div className="mt-2 h-auto w-full flex flex-col items-start">
                        <div className="flex flex-col sm:flex-row sm:justify-between w-full gap-2 sm:gap-0"><h1 className="font-semibold font-albertsans text-[18px] sm:text-[20px] text-[#1f1f1f]">{order.patientorderambherproductname}</h1> <span className={`${formatorderstatusColor(order.patientorderambherstatus)} w-fit font-albertsans font-semibold rounded-full text-[14px] sm:text-[15px] leading-5 px-4 py-2 inline-flex`}>{order.patientorderambherstatus}</span> </div>
                        <div className="mt-5 w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-[#323232] font-semibold text-[13px]">
                          <div className="flex items-center gap-1"><i className="text-[#565656] bx bxs-calendar mt-0.5 font-semibold text-[20px] sm:text-[22px]"/><div><p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">Date Ordered</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">{formatorderDates(order.createdAt)}</p></div></div>
                          <div className="flex items-center gap-1"><i className="text-[#565656] bx bxs-calendar mt-0.5 font-semibold text-[20px] sm:text-[22px]"/><div><p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">Pickup at Ambher Optical</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">{order.patientorderambherproductpickupstatus === 'Now'  ? `Completed (${formatorderDates(order.createdAt)})` : order.patientorderambherproductchosenpickupdate && order.patientorderambherproductchosenpickupdate !== 'Later' && order.patientorderambherproductchosenpickupdate !== 'Now' ? `Available for Pickup: ${formatorderDates(order.patientorderambherproductchosenpickupdate)}` : "To be scheduled"}</p></div></div>
                          <div className="flex items-center gap-1"><i className="text-[#565656] bx bxs-package mt-0.5 font-semibold text-[20px] sm:text-[22px]"/><div><p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">Quantity</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">x{order.patientorderambherproductquantity}</p></div></div>
                          <div className="flex items-center gap-1"><p className="font-semibold text-[20px] sm:text-[22px] text-[#565656]">₱</p><div> <p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">{Number(order.patientorderambheramountpaid) < Number(order.patientorderambherproducttotal) ? (<span className="px-1 py-.5 bg-yellow-100 text-yellow-900 font-alberstans rounded-md text-xs">Down Payment</span> ): "Amount Paid"}</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">{Number(order.patientorderambheramountpaid).toLocaleString('en-PH', {minimumFractionDigits: 2,  maximumFractionDigits: 2})}</p></div></div>
                        </div>
                        <div className="flex items-center justify-between border-t-2 w-full h-10 mt-5">
                          <div></div>
                          <div className="flex items-center gap-3 mt-5 h-auto"><h1 className="font-semibold font-albertsans text-[#343434] text-[16px] sm:text-[17px]">Total Price: </h1><p className="font-semibold font-albertsans text-[22px] sm:text-[25px] text-[#549013]">₱{(order.patientorderambherproductprice * order.patientorderambherproductquantity).toLocaleString('en-PH', {minimumFractionDigits: 2,  maximumFractionDigits: 2})}</p></div>
                        </div>
                    </div>
                  </div>
                        ))
          ...
          { activeorderstable === 'bautistaorderstable' && ( <div id="bautistaorderstable" className="p-2 animate-fadeInUp flex flex-col lg:flex-row items-start w-full h-auto lg:h-[83%] rounded-2xl mt-5" >
                ...
                {['All', 'Pending', 'Ready for Pickup', 'Completed'].map(status => {
                    ...
                    return (
                       <div key={status} onClick={() => {setfilterbautistaorderedproductsStatus(status); setsearchpatientorderedProducts('');}} className={`mt-3 hover:rounded-2xl transition-all duration-300 ease-in-out border-2 b-[#909090] rounded-3xl py-2 text-center flex justify-center items-center cursor-pointer ${filterbautistaorderedproductsStatus === status ? 'bg-[#2781af] rounded-2xl text-white' : ''}`} >
                        <h1 className={`font-albertsans font-semibold text-sm sm:text-base ${filterbautistaorderedproductsStatus === status ? 'text-white' : 'text-[#1f1f1f]'}`}>{status} <span className="bg-gray-200 text-gray-500 font-semibold px-2 rounded-full ml-2 text-xs"> {patientorderedstatusCount} </span> </h1>  </div>
                        );})}
          ...
          <div className="flex flex-col justify-start items-start ml-0 lg:ml-2 rounded-2xl w-full lg:w-[80%] min-h-[540px] max-h-auto h-auto shadow-b-lg">
              ...
              <div className="mt-5 w-full rounded-2xl h-auto flex flex-col gap-3 p-2 lg:pl-2 lg:pt-2 lg:pr-30">
                  ...
                  <div key={order.patientorderbautistaid} onClick={() => handleViewOrder(order)} className="pb-7 shadow-md rounded-2xl py-3.25 px-3.25 flex flex-col sm:flex-row items-start sm:items-center motion-preset-slide-up w-full h-auto cursor-pointer hover:shadow-lg transition-all duration-300">
                    <img src={order.patientorderbautistaproductimage?.[0] || 'default-image-url'} alt={order.patientorderbautistaproductname} className="mr-0 sm:mr-5 w-full sm:w-35 h-32 sm:h-35 rounded-2xl mb-4 sm:mb-0 object-cover"/>
                    <div className="mt-2 h-auto w-full flex flex-col items-start">
                        <div className="flex flex-col sm:flex-row sm:justify-between w-full gap-2 sm:gap-0"><h1 className="font-semibold font-albertsans text-[18px] sm:text-[20px] text-[#1f1f1f]">{order.patientorderbautistaproductname}</h1> <span className={`${formatorderstatusColor(order.patientorderbautistastatus)} w-fit font-albertsans font-semibold rounded-full text-[14px] sm:text-[15px] leading-5 px-4 py-2 inline-flex`}>{order.patientorderbautistastatus}</span> </div>
                        <div className="mt-5 w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-[#323232] font-semibold text-[13px]">
                          <div className="flex items-center gap-1"><i className="text-[#565656] bx bxs-calendar mt-0.5 font-semibold text-[20px] sm:text-[22px]"/><div><p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">Date Ordered</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">{formatorderDates(order.createdAt)}</p></div></div>
                          <div className="flex items-center gap-1"><i className="text-[#565656] bx bxs-calendar mt-0.5 font-semibold text-[20px] sm:text-[22px]"/><div><p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">Pickup at Bautista Eye Center</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">{order.patientorderbautistaproductpickupstatus === 'Now'  ? `Completed (${formatorderDates(order.createdAt)})` : order.patientorderbautistaproductchosenpickupdate && order.patientorderbautistaproductchosenpickupdate !== 'Later' && order.patientorderbautistaproductchosenpickupdate !== 'Now' ? `Available for Pickup: ${formatorderDates(order.patientorderbautistaproductchosenpickupdate)}` : "To be scheduled"}</p></div></div>
                          <div className="flex items-center gap-1"><i className="text-[#565656] bx bxs-package mt-0.5 font-semibold text-[20px] sm:text-[22px]"/><div><p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">Quantity</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">x{order.patientorderbautistaproductquantity}</p></div></div>
                          <div className="flex items-center gap-1"><p className="font-semibold text-[20px] sm:text-[22px] text-[#565656]">₱</p><div> <p className="text-[#777777] font-medium text-[12px] sm:text-[13px]">{Number(order.patientorderbautistaamountpaid) < Number(order.patientorderbautistaproducttotal) ? (<span className="px-1 py-.5 bg-yellow-100 text-yellow-900 font-alberstans rounded-md text-xs">Down Payment</span> ): "Amount Paid"}</p><p className="text-[#303030] font-semibold text-[14px] sm:text-[15px]">{Number(order.patientorderbautistaamountpaid).toLocaleString('en-PH', {minimumFractionDigits: 2,  maximumFractionDigits: 2})}</p></div></div>
                        </div>
                        <div className="flex items-center justify-between border-t-2 w-full h-10 mt-5">
                          <div></div>
                          <div className="flex items-center gap-3 mt-5 h-auto"><h1 className="font-semibold font-albertsans text-[#343434] text-[16px] sm:text-[17px]">Total Price: </h1><p className="font-semibold font-albertsans text-[22px] sm:text-[25px] text-[#549013]">₱{(order.patientorderbautistaproductprice * order.patientorderbautistaproductquantity).toLocaleString('en-PH', {minimumFractionDigits: 2,  maximumFractionDigits: 2})}</p></div>
                        </div>
                    </div>
                  </div>
                        ))
