...
const handlePatientSelect = (patient) => {
  ...
  if (patientConversation) {
    ...
    const hasUnread = hasUnreadMessages(patientConversation._id);
    console.log('🔍 Conversation has unread messages:', hasUnread);
    
    // FIXED: Always fetch fresh messages if conversation has unread messages or cache is empty
    // This ensures we get the complete conversation history, not just cached recent messages
    if (hasUnread || !messagesByConversation[patientConversation._id]) {
      console.log('🔄 Loading fresh messages from server (unread messages or no cache)');
      loadMessages(patientConversation._id, false, false).then(() => {
        setLoading(false);
      });
    } else if (messagesByConversation[patientConversation._id]) {
      console.log('💾 Setting messages from cache for staff/owner (no unread messages)');
      setMessages(messagesByConversation[patientConversation._id]);
      setLoading(false);
    } else {
      console.log('🔄 Loading messages from server (fallback)');
      loadMessages(patientConversation._id, false, false).then(() => {
        setLoading(false);
      });
    }
...
// Single consolidated initialization handler - no more duplicates
  ...
  // Routes where chat icon should be hidden
  const hiddenRoutes = ["/", "/userlogin", "/patientregistration"];
  
  // Don't render chat button on hidden routes
      ...
      {(localStorage.getItem("role") === "staff" || localStorage.getItem("role") === "owner") && (localStorage.getItem("staffclinic") === "Ambher Optical" || localStorage.getItem("ownerclinic") === "Ambher Optical") && (
        <div id="ambherchatdashboard" 
          ...
          {showpatientchatdashboard && (
            <div style={{
              marginBottom: '24px',
              width: window.innerWidth < 768 ? `${window.innerWidth - 20}px` : '1100px',
              maxWidth: '1100px',
              height: window.innerWidth < 768 ? `${window.innerHeight - 100}px` : '630px',
              maxHeight: '630px',
              boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)',
              zIndex: 9999,
              display: 'flex',
              ...
              {/* Modern Header */}
              <div className="flex items-center justify-between px-4 py-5 border-b-2 border-[#E2E8F0] bg-[#ffffff]">
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center">
                    <img src={ambherlogo} alt="Ambher Optical Logo" className="w-8 h-8 object-contain"/>
                  </div>
                  <div>
                    <h3 className="text-black font-semibold text-lg">Ambher Optical</h3>
                ...
                <button 
                  ...
                  onMouseLeave={(e) => e.target.style.backgroundColor = 'rgba(255, 255, 255, 0.1)'}
                >
                  <i className="select-none bx bx-x text-gray-600 text-xl"></i>
                </button>
              </div>
              ...
              <div style={{
                ...
                padding: '16px',
                gap: '16px',
                minHeight: '0' /* Crucial for proper flex behavior */
              }}>
                ...
                <div className="w-1/4 flex flex-col space-y-4 ">
                  ...
                  <div className="flex space-x-2">
                    <button 
                      ...
                      style={{
                        flex: '1',
                        padding: '8px 16px',
                        backgroundColor: activeambhermessageslist === 'allambhermessageslist' ? '#10b981' : 'transparent',
                        color: activeambhermessageslist === 'allambhermessageslist' ? 'white' : '#6b7280',
                        border: '1px solid #e5e7eb',
                        borderRadius: '12px',
                        fontSize: '14px',
                        fontWeight: '500',
                      ...
                      onMouseEnter={(e) => {
                        if (activeambhermessageslist !== 'allambhermessageslist') {
                    ...
                    <button 
                      ...
                      style={{
                        flex: '1',
                        padding: '8px 16px',
                        backgroundColor: activeambhermessageslist === 'unreadambhermessageslist' ? '#10b981' : 'transparent',
                        color: activeambhermessageslist === 'unreadambhermessageslist' ? 'white' : '#6b7280',
                        border: '1px solid #e5e7eb',
                        borderRadius: '12px',
                        fontSize: '14px',
                        fontWeight: '500',
                  ...
                  <div style={{
                    flex: '1',
                    overflowY: 'auto',
                    minHeight: '0' /* Crucial for proper flex scrolling */
                  }} className="space-y-2">
                    {loadingConversations ? (
                      ...
                      <>
                        ...
                        <div 
                          className="relative p-3 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:shadow-md transition-all duration-200 cursor-pointer"
                          onClick={() => {
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-3">
                            <div className="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center">
                              <img src={bautistalogo} alt="Bautista Eye Center Logo" className="w-8 h-8 object-contain"/>
                            </div>
                            <div className="flex-1">
                              <h4 className="font-semibold text-gray-900 text-sm">Bautista Eye Center</h4>
                        ...
                        {getFilteredPatientsForDisplay()
                          ...
                          .map(({ patient }) => (
                            <div 
                              key={patient._id}
                              className={`relative p-3 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:shadow-md transition-all duration-200 cursor-pointer ${
                                selectedPatient?._id === patient._id ? 'bg-emerald-50 border-emerald-200' : ''
                              }`}
                              onClick={() => {
                ...
                {selectedPatient === null && selectedClinic === null ? (
                  <div className=" flex-1 flex items-center justify-center bg-gray-50 rounded-xl">
                    <div className="text-center space-y-3">
                      <div className="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center mx-auto">
                        <i className="bx bx-message-dots text-gray-400 text-2xl"></i>
                      </div>
                      <div>
                        <h4 className="text-gray-700 font-medium">No conversation selected</h4>
                        <p className="text-gray-500 text-sm">Choose a patient or clinic to start messaging</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div style={{
                    ...
                    <div style={{
                      ...
                      flexShrink: '0' /* Prevents header from being squeezed */
                    }}>
                      <div className="flex items-center space-x-3">
                        <img 
                          src={
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {selectedPatient ? 'Patient' : 'Clinic'}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      {(loading || loadingMessages[conversationId]) ? (
                        <div className="flex flex-col items-center justify-center h-full space-y-4">
                          ...
                          <p className="text-emerald-600 font-medium">Loading messages...</p>
                        </div>
                      ) : messages.length > 0 ? (
                                  ...
                                  {isImageOnly ? (
                                    ...
                                    <div 
                                      className={`flex flex-col px-4 py-2 ${
                                        isCurrentClinic ? 'bg-[#c0eed6]' : 'bg-[#e0e0e0]'
                                      } ${borderRadiusClasses} relative group`}
                                      style={{
                                        wordWrap: 'break-word',
                                        overflowWrap: 'break-word',
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-sm">
                              {selectedPatient ? `Start a conversation with ${selectedPatient.patientfirstname} ${selectedPatient.patientlastname}!` : 'Start the conversation!'}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    <div style={{
                      ...
                      paddingTop: '16px',
                      flexShrink: '0' /* Prevents input area from being squeezed */
                    }}>
                      <div className="bg-gray-50 rounded-2xl p-3 flex items-end space-x-3 min-h-[60px] transition-all duration-200 focus-within:bg-gray-100 focus-within:shadow-sm">
                        {/* Attachment Button */}
                        {!selectedFile && (
          ...
          <div className="w-full justify-end flex items-end">
            {showpatientchatdashboard ? (
              ...
              >
                ...
                <i className="bx bx-message-dots text-white text-2xl"></i>
              </button>
            )}
          </div>
        </div>
                  ...
                  <div className="flex space-x-2">
                    <button 
                      ...
                      style={{
                        flex: '1',
                        padding: '8px 16px',
                        backgroundColor: activeambhermessageslist === 'allambhermessageslist' ? '#3b82f6' : 'transparent',
                        color: activeambhermessageslist === 'allambhermessageslist' ? 'white' : '#6b7280',
                        border: '1px solid #e5e7eb',
                        borderRadius: '12px',
                        fontSize: '14px',
                    ...
                    <button 
                      ...
                      style={{
                        flex: '1',
                        padding: '8px 16px',
                        backgroundColor: activeambhermessageslist === 'unreadambhermessageslist' ? '#3b82f6' : 'transparent',
                        color: activeambhermessageslist === 'unreadambhermessageslist' ? 'white' : '#6b7280',
                        border: '1px solid #e5e7eb',
                        borderRadius: '12px',
                        fontSize: '14px',
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1">
                              <h4 className="font-semibold text-gray-900 text-sm">Ambher Optical</h4>
                              <p className="text-xs text-gray-500 truncate">
                                {(() => {
                                  const clinicConversation = conversations.find(conv => 
                                    conv.participants.some(p => p.role === 'clinic' && p.clinic === "Bautista Eye Center") &&
                                  ...
                                  const latestMessage = clinicConversation ? getLatestMessageForConversation(clinicConversation._id) : null;
                                  return getLatestMessageDisplay({ patientfirstname: "Bautista Eye Center" }, latestMessage ? [latestMessage] : [], clinicConversation?._id);
                                })()}
                              </p>
                            </div>
                          </div>
                                ...
                                <div className={`max-w-[80%] ${isCurrentClinic ? 'order-0' : 'order-1'}`}>
                                  ...
                                  {isImageOnly ? (
                                    ...
                                    <div 
                                      className={`flex flex-col px-4 py-2 ${
                                        isCurrentClinic 
                                          ? (localStorage.getItem("staffclinic") === "Bautista Eye Center" || localStorage.getItem("ownerclinic") === "Bautista Eye Center")
                                            ? 'bg-[#d8f1fd]'
                                            : 'bg-[#c0eed6]'
                                          : 'bg-[#e0e0e0]'
                                      } ${borderRadiusClasses} relative group`}
